@if(auth()->check())
<div class="alert alert-info m-b-none">@lang('articles::article.message.need_to_buy', ['amount' => $article->cost])</div>
...
@else
    <div class="alert alert-warning m-b-none">
        @lang('articles::article.message.need_to_register', ['amount' => $article->cost])
        @if($couponsCount > 0)
            <br />
            <small>
